function graphIt(){
	...
	$.getJSON('/api', function(results){
		...
		var minDate = d3.min(data, function(c) { return d3.min(c.stock_array, function(v) { return getDate(v.date); }); });
		var maxDate = d3.max(data, function(c) { return d3.max(c.stock_array, function(v) { return getDate(v.date); }); });
		x.domain([
